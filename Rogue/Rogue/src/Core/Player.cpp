...
void Player::UpdateFirstPersonWeaponLogicAndAnimations(float deltaTime) {
	...
	if (_currentWeaponIndex == Weapon::GLOCK) {
		...
		// Reload
		if (!_ignoreControl && Input::KeyPressed(HELL_KEY_R) && _inventory.glockAmmo.total > 0 && _inventory.glockAmmo.clip < GLOCK_CLIP_SIZE && !isReloading) {
	...
	if (_currentWeaponIndex == Weapon::AKS74U) {
		...
		if (_weaponAction == RELOAD || _weaponAction == RELOAD_FROM_EMPTY) {
			if (needsAmmoReloaded&& _firstPersonWeapon.AnimationIsPastPercentage(38.0f)) {
		...
		// Reload	
		if (!_ignoreControl && Input::KeyPressed(HELL_KEY_R) && _inventory.aks74uAmmo.total > 0 && _inventory.aks74uAmmo.clip < AKS74U_MAG_SIZE && !isReloading) {
			isReloading = true;

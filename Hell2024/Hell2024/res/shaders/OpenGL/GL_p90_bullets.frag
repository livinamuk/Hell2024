...
uniform int bulletBaseColorTextureIndex;
uniform int bulletNormalTextureIndex;
uniform int bulletRMATextureIndex;
...
void main() {
	...
	vec4 baseColor = texture(sampler2D(textureSamplers[bulletBaseColorTextureIndex]), TexCoords);
    vec4 normalMap = texture(sampler2D(textureSamplers[bulletNormalTextureIndex]), TexCoords);
    vec4 rma = texture(sampler2D(textureSamplers[bulletRMATextureIndex]), TexCoords);
	...
	FragOut.rgb = pow(FragOut.rgb, vec3(1.0/2.2));
	FragOut.rgb = mix(FragOut.rgb, Tonemap_ACES(FragOut.rgb), 0.1235);

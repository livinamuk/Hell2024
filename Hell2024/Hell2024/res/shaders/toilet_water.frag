...
void main()
{	
	vec3 normalMap = texture(normalMap, TexCoords*0.15 + 0.5).rgb * 2 - 1; 
	vec3 normal;
	normal.x = normalMap.x;
	normal.y = normalMap.z;
	normal.z = normalMap.y;

	fragOut.rgba = vec4(0.3, 0.3, 0.3, 0.05);
	fragOut.rgb = normal;
	fragOut.a = 1.0;
    ...
    for(int i = 0; i < lightsCount; i++) {
        vec3 ligthting = GetDirectLighting(lights[i].position, lights[i].color, lights[i].radius, lights[i].strength * 2.0, normal, WorldPos.rgb, baseColor.rgb, roughness, metallic);

...
OpenGLMesh::OpenGLMesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices, std::string name, const bool instant_bake)
    : _name{ std::move(name) }
...
int OpenGLMesh::GetIndexCount() {
    return static_cast<int>(indices.size());
}

int OpenGLMesh::GetVAO() {
    return _VAO;
}

void OpenGLMesh::CreateTriangleMesh() {
...
void OpenGLMesh::CreateConvexMesh() {
    std::vector<PxVec3> pxvertices;
    for (auto& vertex : vertices) {
        bool found = false;

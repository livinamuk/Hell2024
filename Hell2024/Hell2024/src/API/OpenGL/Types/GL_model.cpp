#include "GL_model.h"
#include "../../../Util.hpp"
#include "../../../API/Vulkan/VK_util.hpp"
...
void OpenGLModel::Load(std::string filepath, const bool bake_on_load) {
...
void OpenGLModel::CreateVulkanModel(const char* filepath, VulkanModel& vulkanModelOut) {
    ...
    VulkanFileInfo fileinfo = VulkanUtil::GetFileInfo(filepath);
    vulkanModelOut._filename = fileinfo.filename;
    ...
    for (const auto& shape : shapes)
    {
        ...
        for (int i = 0; i < shape.mesh.indices.size(); i++) {
            ...
            if (attrib.texcoords.size() && index.texcoord_index != -1) { // should only be 1 or 2, some bug in debug where there were over 1000 on the sphere lines model...
        ...
        VulkanUtil::SetTangentsFromVertices(vertices, indices);

        int meshIndex = VulkanAssetManager::CreateMesh(vertices, indices);
        vulkanModelOut._meshIndices.push_back(meshIndex);
        VulkanAssetManager::GetMesh(meshIndex)->_name = shape.name;
        vulkanModelOut._meshNames.push_back(shape.name);
    }
}
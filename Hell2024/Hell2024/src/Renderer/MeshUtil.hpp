...
namespace MeshUtil {
	...
	inline OpenGLMesh CreateUpFacingPlane(float xMin, float xMax, float zMin, float zMax, float height) {
        ...
        indices.push_back(2);
        indices.push_back(1);
        return OpenGLMesh(vertices, indices, "UpPlane");
	}

	inline OpenGLMesh CreateDownFacingPlane(float xMin, float xMax, float zMin, float zMax, float height) {
        float width = xMax - xMin;
        float depth = zMax - zMin;
        std::vector<Vertex> vertices;
        std::vector<unsigned int> indices;
        ...
        indices.push_back(2);
        indices.push_back(3);
        return OpenGLMesh(vertices, indices, "DownPlane");
	}

    inline OpenGLMesh CreateCube(float size, float textureScaling = 1.0f, bool outFacing = true) {
        std::vector<Vertex> vertices;
        std::vector<unsigned int> indices;
        float d = size * 0.5f;
        // Top
        ...
        return OpenGLMesh(vertices, indices, "Cube");
    }

    inline OpenGLMesh CreateCuboid(float width, float height, float depth, float textureScaling = 1.0f, bool outFacing = true) {
        std::vector<Vertex> vertices;
        std::vector<unsigned int> indices;
        float w = width * 0.5f;
        float h = height * 0.5f;
        ...
        if (outFacing)
            ...
            indices = { 0, 1, 3, 1, 2, 3, 7, 5, 4, 7, 6, 5, 11, 9, 8, 11, 10, 9, 12, 13, 15, 13, 14, 15, 16, 17, 19, 17, 18, 19, 23, 21, 20, 23, 22, 21 };
        return OpenGLMesh(vertices, indices, "Cuboid");
    }    
}